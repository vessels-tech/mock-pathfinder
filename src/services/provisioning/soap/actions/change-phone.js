    ...
    return Validation.validateChangePhoneStatusRequest(params)
      ...
      .then(request => {
        return PhoneService.getByNumber(request.phone.number, request.phone.countryCode)
          .then(existingPhone => {
            if (!existingPhone) {
              throw new Errors.InvalidValueError(`TN ${request.phone.number} WAS NOT PROVISIONED`)
            }
            ...
            return ProfileService.getByName(request.profileName)
              .then(foundProfile => {
                ...
                return PhoneService.update(existingPhone.phoneId, { profileId: foundProfile.profileId, status: request.status })
                  .then(created => Result.build(200, transactionId, ['OK', 'TN profile updated successfully']))
              })
          })
      })
